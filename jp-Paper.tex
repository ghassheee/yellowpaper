...
私は形式的な表記に際し、数々の書式の慣習を使用します。
そのうちのいくつかは、現在の仕事に特化したものです。:


高度に構造化されたtop-levelの状態値（抽象変数）である２つの集合は
ギリシア文字の太い小文字で表されます。
これらはworld-stateの集合 $\boldsymbol{\sigma}$ と
machine-stateの集合 $\boldsymbol{\mu}$ に仕分けされます。

高構造化された値を扱う関数はギリシア語の大文字で表記します。
例えば、$\Upsilon$ はEthereum状態遷移関数です。

多くの関数には、大文字を使います。例として、$C$は一般コスト関数です。
特別変数として異なる関数を表す場合は、例えば、$C_\text{\tiny SSTORE}$は
{\tiny SSTORE}命令のコスト関数、のようにします。

外部定義可能な特別な関数は、例えば、
Keccak-256ハッシュ関数（SHA-3コンテストで優勝した暗号関数による）は$\texttt{KEC}$とします。
（また、単にKeccakと表記）
同様に$\texttt{KEC512}$はKeccak 512ハッシュ関数を表します。

タプルは大文字で表記される典型的なもので、
例えば、$T$はEthereumトランザクションの表記に使います。
この記号には、状況に応じて定義され、ひとつの要素を参照するような二次的表記あり、
例えば、$T_n$は当該トランザクションのnonceを表します。
この二次表記形式は、型の表記にも使用されます。
例として大文字の二次表記は、タプル中の二次表記可能な要素を参照します。

スカラー値や固定長バイト列は標準書式の小文字で表記され、
例えば、$n$はトランザクションのnonceの表記に使用されます。
ある特別な意味合いを持たせたい場合はギリシア文字を用い、例えば、
$\delta$は、与えられた命令でスタック上で必要とされるアイテムの数です。

任意長文字列は、ふつう
太い小文字で表記され、
例えば、$\mathbf{o}$は、メッセージコールの出力値であるバイト列の表記に使われます。
特に重要な値には、太い大文字が使われます。

一貫して、スカラー値は正の整数値であり、つまり集合$\mathbb{P}$の要素となる。
バイト列の全集合は$\mathbb{B}$であり、Appendix \ref{app:rlp}で形式的に定義されている。
それらのデータの集合がある特定の長さである場合、二次表記を用いて、
全バイト列長が$32$である場合には$\mathbb{B}_{32}$と表記され、
$2^{256}$より小さい正整数の全集合は、$\mathbb{P}_{256}$と命名しています。
これは、\ref{ch:block}章で形式的に定義します。

角型の括弧は個々の要素や2次列の参照に使用されます
。
例 $\boldsymbol{\mu}_\mathbf{s}[0]$はmachine stack上の最初のアイテムを表します。
二次列には、範囲を表すのに省略符号が使用され、両端の極限値も含みます。
例 $\boldsymbol{\mu}_\mathbf{m}[0..31]$はmachine memoryの最初の32個のアイテムを表します。

global state $\boldsymbol{\sigma}$の場合、
それはアカウント列であり、それ自身タプルであり、
角型の括弧は、個々のアカウントを参照するために使います。

ある値の細かな違いを考えるとき、
あるスコープを定義に加えた規則に従います。
前処理の施していない入力値が$\Box$とすると
$\Box'$は処理を施し使える状態となった値を表します。
また中間の値は$\Box^*$や$\Box^{**}$などと表記します。
特別な状況に置いては、可読性を高めるためにあるいは意味合いの一義性を確保するために、
アルファ数字の二次表記を中間の値を表すのに使用します。
これは、非常に注意してほしい場合におけるものです。

ある$f$という関数を考える場合、$f^*$は、引数の範囲がデータ列に限らない、
よく似た、引数の幅が広い、より抽象化された関数写像を表します。
\ref{ch:block}章で形式的に定義します。

私は始終、幾多の役立つ関数を定義します。
よくつかうものとして$\ell$があり、与えられたデータ列の最後のアイテムを評価し、返します。
...
Ethereumの背景にある概念を説明してきたので、transaction, block, stateの意味について詳細に議論する。
...
world state (\textit{state})はアドレス（160バイトの識別子）から、
アカウントの状態（RLPとして識別番号が付与されたデータ構造、Appendix \ref{app:rlp}参照）を返す写像です。
ブロックチェーン上にはstoreされませんが、
実装に際し、この写像は、パトリシア木(\textit{trie}, \ref{app:trie}参照)の中で保持されます。
このtrieは、バイト配列からバイト配列への写像を保持するデータベースバックエンドを要します。
このデータベースをstateデータベースと命名します。
これにはたくさんの利点があります。
まずこのtrie構造のroot nodeは、暗号学的にすべての内部データに依存しており、それが故、
そのハッシュ値は全システムの状態を強固に管理する識別番号として利用できます。
つぎに、変更が加えられることのないデータ構造なので、
root hashを単に置き換えることにより、それ以前のすべての状態を呼び出すことができます。
ブロックチェーン上に以前のroot hashをstoreしてあるので
些細な変更で前の状態に戻すことができます。

アカウントの状態は次の4つのフィールドにより構成されます。

\begin{description}
\item[nonce]
スカラー値で、このアカウントのアドレスから送られたトランザクション番号に等しい。
あるいは、アカウントがコードと連携している場合は、
このアカウントが作成するcontract-creations番号に等しい。
アカウントのアドレスが$a$で、状態$\boldsymbol{\sigma}$にあるとき、
これは形式的に$\boldsymbol{\sigma}[a]_n$と表記します。
\item[balance]
スカラー値で、アカウントのアドレスにより所有されるWeiの数量に等しい。
形式的に$\boldsymbol{\sigma}[a]_b$と表記します。
\item[storageRoot]
アカウントのstorageの中身を符号化した
パトリシア木のルートノードの256bitハッシュ値を格納した配列（256bit integer 値間の写像）です。
この値は、
「256bit Integer値の鍵をKeccak256bitハッシュにかけた値から、RLP符号化された256bit Integer値への写像」として、
trieの中に符号化されています。
アドレスを入力値としてストレージルートを返す。
このハッシュ値は形式的に$\boldsymbol{\sigma}[a]_s$と表記します。
\item[codeHash]
アカウントのEVM codeのハッシュ値です---
ここで言うコードはアカウントのアドレスがメッセージコールをうけとると実行されるコードです
これは変更不可能です。他の３つのフィールドと異なり、コンストラクションの後は一切変更できません。
コードの断片はすべて、それら各々のハッシュ値の下にあるstateデータベースに含まれ、
最新のデータ取得ができます。
このハッシュ値は形式的には、$\boldsymbol{\sigma}[a]_c$と表記し、
ここでいうところのコードを$\mathbf{b}$とすると、
$\texttt{\small KEC}(\mathbf{b}) = \boldsymbol{\sigma}[a]_c$で与えられます。
\end{description}

trieのルートハッシュを参照するのでなく、内部にあるkey/valueペアを参照したいので、
次のように簡略化した表記を定義します。
...
trieにおけるkey/valueペアを引数にとる崩壊関数である$L_I^*$は、
基礎となる関数$L_I$の要素の幅が広い変換したものとして定義されます。
