...
ここで、$\Upsilon$  : Ethereum 状態遷移関数

Ethereum, つまり $\Upsilon$ は、 $\boldsymbol{\sigma}$ と一緒になることで現存の他の競合するシステムにないより強力なものとなります。
...
They also punctuate the transaction series with incentives for nodes to \textit{mine}.
This incentivisation takes places as a state-transition function,
adding value to a nominated account.

Mining is the process of dedicating effort (working)
to bolster one series of transactions (a block) over any other potential competitor block. It is achieved thanks to a cryptographically secure proof. This scheme is known as a proof-of-work and is discussed in detail in section \ref{ch:pow}.
